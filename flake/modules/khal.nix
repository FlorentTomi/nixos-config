# Dendrite for khal. Content is moved verbatim from the old modules/khal.nix
# — this is pure relocation, not a rewrite. Exposed as
# `flake.modules.nixos.khal` so hosts can pull it in via `flakeModules`
# (threaded through lib/mk-host.nix's specialArgs).
{ ... }:
{
  flake.modules.nixos.khal =
    { config, lib, ... }:
    let
      cfg = config.modules.khal;
    in
    {
      options.modules.khal = {
        enable = lib.mkEnableOption "khal + vdirsyncer Google Calendar sync";
      };
